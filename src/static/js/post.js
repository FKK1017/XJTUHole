...
function main() {
    ...
    $.ajax(url + '/gettopic', {
		...
		window.author = data['author_name'];
        var co = document.getElementById("com");
        if (data['collect']) {
            ...
            for (var i = 0; i < l; i++) {
                ...
                if (data['comment'][i]['like'] == true) {
                    co.innerHTML = co.innerHTML + ' <li id="' + data['comment'][i]['comment_id'] + '"><div class="comment1"><div class="HeadBar1">' +
                        ...
                        '<li id="cmt-nav' + data['comment'][i]['comment_id'] + '"><a href="#picture" class="mui-btn mui-btn-primary">举报</a></li></ul></div><p class="time1">' + data['comment'][i]['comment_time'] + '</p></div>' +
        ...
        $.ajax(url + '/upcomment', {
            ...
            if (data) {
                ...
                co.innerHTML = co.innerHTML + ' <li id="' + data['comment_id'] + '"><div class="comment1"><div class="HeadBar1">' +
                    ...
                    '<li id="cmt-nav' + data['comment_id'] + '"><input type="button" value="举报"></li>' +
                    '</ul></div>' +
